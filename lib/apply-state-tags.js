...
function applyStateTags(cleanResponse, gameState, worldConfig, matchLocationId) {
  ...
  // Clue/lead tracking ("Leads" in the UI) — investigation-shaped state,
  // separate from Objectives' goal-shaped tracking, but the same shared-
  // across-all-world-types scope and the same objectivesMatch() fuzzy dedup
  // (reused verbatim rather than a second matcher). No "contradicts clue X"
  // relationship yet — deliberately deferred until real transcripts justify
  // the added risk, same posture as everything else in this table.
  if (!gameState.worldState.clues) gameState.worldState.clues = [];
  const clueUpdates = extractClueUpdates(cleanResponse, gameState.worldState.clues);
  for (const text of clueUpdates.additions) {
    gameState.worldState.clues.push({ text, done: false });
  }
  for (const text of clueUpdates.resolvedTexts) {
    const clue = gameState.worldState.clues.find(c => c.text === text);
  ...
  if (worldConfig.id === "manlandia" || worldConfig.type === "custom") {
    ...
    // A named bonus ability gets its own once-per-session used tracking
    // (character.bonus_abilities_used), separate from the starting ability's
    // ability_used boolean — see extractAbilityUsedKeys' comment for how the
    // two are told apart.
    for (const { key, bonusAbilityId } of extractAbilityUsedKeys(cleanResponse, gameState.characters)) {
      const character = gameState.characters[key];
      if (bonusAbilityId) {
        if (!character.bonus_abilities_used) character.bonus_abilities_used = [];
        if (!character.bonus_abilities_used.includes(bonusAbilityId)) {
          character.bonus_abilities_used.push(bonusAbilityId);
        }
      } else {
