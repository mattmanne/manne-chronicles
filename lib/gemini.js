...
async function callGroq(model, systemPrompt, history, userMessage, maxTokens = 1024) {
...
async function generateContent(systemPrompt, history, userMessage) {
  try {
    ...
    try {
      ...
      // not a blip. Fall back to a model with its own separate bucket. That
      // bucket is much smaller (a small/fast model's free-tier TPM budget can
      // be a fraction of the 70b model's), so trim the history and response
      // budget hard — a short-context reply is far better than none.
      try {
        return await callGroq(FALLBACK_MODEL, systemPrompt, history.slice(-6), userMessage, 400);
      } catch (_fallbackErr) {
        // Whatever went wrong with the fallback (also rate-limited, too
        // large even trimmed, model renamed, etc.), the primary's 429 — with
        // its known retry-after — is the clearer error to surface than a
        // confusing fallback failure.
        throw retryErr;
      }
    }
  }
