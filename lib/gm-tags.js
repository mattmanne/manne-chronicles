...
// Every character key doubles as its own "name" for roll-attribution
// purposes (Resonance's fen/lyra have no .name field at all — the key IS
// the name), on top of whatever buildNameToKeyMap already resolves from a
// custom .name field (Manlandia/custom heroes).
function buildRollerNameMap(characters) {
  const map = buildNameToKeyMap(characters);
  for (const key of Object.keys(characters || {})) map[key.toLowerCase()] = key;
  return map;
}

// Pulls the first run of letters out of a colon-separated token, ignoring
// any surrounding brackets/punctuation — e.g. "[AGILITY]" or "AGILITY," both
// yield "AGILITY". Mirrors the old single-word-capture regex's tolerance.
function firstWord(token) {
...
// case. Also tolerates an extra colon-separated token naming which
// character is rolling (new: "ROLL:FEN:AGILITY" or "ROLL:AGILITY:FEN", order
// doesn't matter) — needed once a merged multi-character turn means there's
// no longer a single implicit submitter to attribute the roll to. A token
// that resolves to neither a stat, ADVANTAGE, nor a known character name
// safely falls through and keeps scanning for another occurrence, same as
// an unrecognized stat word always has.
function extractRoll(rawText, characters) {
  const nameToKey = buildRollerNameMap(characters);
  let found = null;
  const rollAt = /ROLL:/gi;
  let m;
  ...
  while ((m = rollAt.exec(rawText)) !== null) {
    const startAt = m.index + m[0].length;
    // Bounded to this occurrence only — stops at the next line break, the
    // next "ROLL:" mention (so two triggers close together never bleed into
    // each other's token scan), or 40 chars, whichever comes first.
    let endAt = rawText.length;
    const nl = rawText.indexOf("\n", startAt);
    if (nl !== -1) endAt = Math.min(endAt, nl);
    const nextRoll = rawText.slice(startAt).search(/ROLL:/i);
    if (nextRoll !== -1) endAt = Math.min(endAt, startAt + nextRoll);
    endAt = Math.min(endAt, startAt + 40);

    let stat = null, rollPlayer = null, advantage = false;
    for (const token of rawText.slice(startAt, endAt).split(":")) {
      const word = firstWord(token);
      if (!word) continue;
      if (word === "ADVANTAGE") { advantage = true; continue; }
      if (!stat && STAT_SYNONYMS[word.toLowerCase()]) { stat = STAT_SYNONYMS[word.toLowerCase()]; continue; }
      if (!rollPlayer && nameToKey[word.toLowerCase()]) { rollPlayer = nameToKey[word.toLowerCase()]; continue; }
    }
    if (stat) { found = { stat, advantage, rollPlayer }; break; }
  }

  // Fallback for when the model asks for a roll in plain prose with no
  // "ROLL:" anchor at all — live examples: "Roll a FORCE to see how well
  // Kestra can hold her ground", "Rolling Bram's Force for the attack". Every
  // prior fix to this function assumed the literal substring "ROLL:" would
  // be present somewhere in the text (just in an unexpected shape); this is
  // a more extreme drift where that anchor is simply missing, so no amount
  // of tolerance on that token could ever have caught it — confirmed live,
  // independently, by 6 separate playtest sessions across 2 campaigns
  // (2026-07-05). Only runs when the primary scan above found nothing, so an
  // explicit ROLL: tag always takes priority. Deliberately NOT stripped from
  // `clean` below — unlike the raw ROLL: tag syntax, this is legitimate
  // prose a player should actually read, not a mechanical signal leaking in.
  if (!found) {
    const naturalRe = /\bRoll(?:ing)?\b(?!\s+of\b)\s+(?:(?:the|an?|your)\s+)?(?:([A-Za-z]+)'s\s+)?([A-Za-z]+)/gi;
    let nm;
    while ((nm = naturalRe.exec(rawText)) !== null) {
      const stat = STAT_SYNONYMS[nm[2].toLowerCase()];
      if (!stat) continue;
      const rollPlayer = nm[1] ? (nameToKey[nm[1].toLowerCase()] || null) : null;
      found = { stat, advantage: false, rollPlayer };
      break;
    }
  }
  ...
  return {
    ...
    rollPlayer: found ? found.rollPlayer : null,
  };
}
...
//
// Also tolerates two more live-observed paddings, both from the same
// response: a "(Name)" parenthetical inserted between the number and the
// colon ("[CHARACTER 1 (Kestra): ...]"), and a literal "Harm:" label before
// the harm word itself ("[CHARACTER 1 (Kestra): Harm: Unhurt]") — neither
// the arrow nor the single-value regex below could match either padding
// before this, so a real harm change written in this shape would have
// silently no-op'd.
const CHARACTER_PADDING = "(?:\\s*\\([^)]*\\))?:\\s*(?:Harm:\\s*)?";
function extractCharacterHarmUpdates(text, characters) {
  ...
  const numberedRe = new RegExp(`\\[CHARACTER (\\d)${CHARACTER_PADDING}([A-Za-z]+)\\s*${ARROW}([A-Za-z]+)[^\\]]*\\]`, "gi");
  let m;
  ...
  const numberedSingleRe = new RegExp(`\\[CHARACTER (\\d)${CHARACTER_PADDING}([A-Za-z]+)\\s*\\]`, "gi");
...
//
// Once a hero has unlocked bonus abilities (see lib/growth.js), "used" is
// ambiguous — which of their powers fired? The padding is the only signal:
// if it names one of that hero's *bonus* ability ids (matched loosely by the
// id's own words, e.g. "lucky_break" -> "lucky break"), attribute the use to
// that bonus ability instead of the starting one. No name, or a name that
// doesn't match a bonus ability, falls back to the original starting-ability
// behavior unchanged — this is the only case there's live evidence for so far.
//
// Also tolerates a "(Name)" parenthetical inserted between the number and
// the colon — live: "[ABILITY 1 (Kestra): Protect Friend used]" — which the
// original digit-then-colon regex couldn't match at all, silently dropping a
// real ability use (confirmed live: ability_used stayed false afterward).
function extractAbilityUsedKeys(text, characters) {
  const re = /\[ABILITY (\d)(?:\s*\([^)]*\))?: (?!.*\bnot\s+used\b)([^\]]*)\bused\b[^\]]*\]/gi;
  const results = [];
  const seen = new Set();
  let m;
  while ((m = re.exec(text)) !== null) {
    ...
    const padding = (m[2] || "").toLowerCase();
    const bonusAbilityId = (character.bonus_abilities || []).find((id) =>
      padding.includes(id.replace(/_/g, " "))
    ) || null;
    const dedupeKey = `${key}:${bonusAbilityId || ""}`;
    if (seen.has(dedupeKey)) continue;
    seen.add(dedupeKey);
    results.push({ key, bonusAbilityId });
  }
  return results;
}
...
// [CLUE: description] / [CLUE RESOLVED: description] — an investigation
// thread ("who lied," "what's in the ledger"), separate from OBJECTIVE's
// goal-shaped tracking ("find the sword"). Deliberately reuses
// objectivesMatch() verbatim rather than inventing a second fuzzy-matcher —
// same paraphrase-tolerance need, same "ship simple, harden from real
// transcripts" posture as every tag in this file. Shared across all world
// types, same as Objectives/NPCs, not gated to any one world.
function extractClueUpdates(text, existingClues) {
  const clues = existingClues || [];
  const additions = [];

  const addRe = /\[CLUE:\s*([^\]]+)\]/gi;
  let m;
  while ((m = addRe.exec(text)) !== null) {
    const desc = m[1].trim();
    if (!desc) continue;
    const alreadyKnown = clues.some((c) => objectivesMatch(c.text, desc))
      || additions.some((d) => objectivesMatch(d, desc));
    if (!alreadyKnown) additions.push(desc);
  }

  const resolvedTexts = [];
  const resolvedRe = /\[CLUE RESOLVED:\s*([^\]]+)\]/gi;
  while ((m = resolvedRe.exec(text)) !== null) {
    const desc = m[1].trim();
    if (!desc) continue;
    const match = clues.find((c) => !c.done && objectivesMatch(c.text, desc));
    if (match && !resolvedTexts.includes(match.text)) resolvedTexts.push(match.text);
  }

  return { additions, resolvedTexts };
}
