...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (err.status === 429) {
      return res.status(429).json({ error: `The GM is handling a lot of requests right now — ${formatWaitMessage(err.retryAfterSeconds)} and try again. [DEBUG: ${err.message} | code=${err.code}]` });
    }
