...
module.exports = async function handler(req, res) {
  ...
  const gameContext = worldConfig.id === "manlandia"
    ? buildSystemPromptManlandia(gameState)
    : buildSystemPrompt(gameState);

  const systemPrompt = `You are a friendly helper for a family tabletop RPG game. Your job is to answer questions from players — including children aged 8 and 10 — about the game rules, world, characters, and what is currently happening in their adventure.

Be warm, clear, and encouraging. Use plain words that children can understand. Keep answers to 2–4 sentences unless a longer explanation is clearly needed. If a player seems confused or worried they are doing something wrong, reassure them — there is no wrong way to play.
